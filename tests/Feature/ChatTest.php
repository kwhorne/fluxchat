...
use Kwhorne\FluxChat\Events\NotifyParticipant;
use Kwhorne\FluxChat\Facades\FluxChat;
use Kwhorne\FluxChat\Helpers\Helper;
use Kwhorne\FluxChat\Jobs\BroadcastMessage;
use Kwhorne\FluxChat\Jobs\NotifyParticipants;
use Kwhorne\FluxChat\Livewire\Chat\Chat as ChatBox;
use Kwhorne\FluxChat\Livewire\Chats\Chats as Chatlist;
use Kwhorne\FluxChat\Models\Attachment;
...
describe('Presense', function () {
    ...
    test('it_doesnt_show_upload_trigger_if_attachments_not_enabled', function () {
        ...
        Config::set('fluxchat.allow_media_attachments', false);
        Config::set('fluxchat.allow_file_attachments', false);
    ...
    test('it_shows_upload_trigger_if_any_one_of_attachments_is_enabled', function () {
        ...
        Config::set('fluxchat.allow_media_attachments', true);
        Config::set('fluxchat.allow_file_attachments', false);
    ...
    test('it_shows_file_upload_input_if_enabled', function () {
        ...
        Config::set('fluxchat.allow_media_attachments', false);
        Config::set('fluxchat.allow_file_attachments', true);
...
describe('Validation', function () {
    ...
    test('file attachment count must not exceed value specified in config && it dispatces fluxchat-toast error', function () {

        // set config value
        Config::set('fluxchat.attachments.max_uploads', 13);
        ...
        $request->assertDispatched('fluxchat-toast');
        //     dd($request->errors());

    });

    test('file  size must not exceed value specified in config && it dispatces fluxchat-toast error', function () {

        // set config value

        Config::set('fluxchat.attachments.media_max_upload_size', 125);
        Config::set('fluxchat.attachments.file_max_upload_size', 125);
        $values = ['pdf'];
        Config::set('fluxchat.attachments.file_mimes', $values);
        //
    ...
    test('media size(KB) must not exceed value specified in config && it dispatces fluxchat-toast error', function () {
    ...
    test('media  Mimes must be the ones  specified in config && it dispatces fluxchat-toast error', function () {

        // set config value

        $values = ['png'];
        Config::set('fluxchat.attachments.media_mimes', $values);
        ...
        $request = Livewire::actingAs($auth)->test(ChatBox::class, ['conversation' => $conversation->id])
            ...
            ->call('sendMessage')
            ->assertHasErrors('media.0');
        //  ->assertHasErrors(['media.0'=>__('fluxchat::validation.mimes', ['attribute' => __('fluxchat::chat.inputs.media.label'),'values'=>'png'])]);
...
describe('Box presence test: ', function () {
    ...
    test('it doesnt show auth name if conversation is group unless auth has replied to another you or vice versa ', function () {
        ...
        $auth = User::factory()->create(['name' => 'TestUser']);
        $conversation = $auth->createGroup('My Group');
    ...
    describe('IsWidget:', function () {
        ...
        test('it renders $dispatch("close-chat") BUT not redirect to chats index', function () {
            ...
            Livewire::actingAs($auth)->test(ChatBox::class, ['conversation' => $conversation->id, 'widget' => true])
                ->assertDontSeeHtml('href="'.route(FluxChat::indexRouteName()).'"')
        ...
        test('it doesnt render $dispatch("close-chat") BUT Renders redirect to chats index', function () {
            ...
            Livewire::actingAs($auth)->test(ChatBox::class, ['conversation' => $conversation->id, 'widget' => false])
                ->assertSeeHtml('href="'.route(FluxChat::indexRouteName()).'"')
...
describe('Sending messages ', function () {
    ...
    test('it saves image to storage when created & clears files properties when done', function () {
        ...
        Config::set('fluxchat.attachments.storage_disk', 'public');
        ...
        Storage::disk('public')->assertExists(FluxChat::storageFolder().'/'.$attachment->file_anme);
    });
    ...
    test('it saves file visibility as public when storage_disk is public', function () {
        ...
        $attachment = Attachment::first();
        $visibility = Storage::disk('public')->getVisibility(FluxChat::storageFolder().'/'.$attachment->file_anme);
    ...
    test('it saves file visibility as public when storage_disk is s3', function () {
        ...
        Config::set('fluxchat.attachments.storage_disk', 's3');
        Config::set('fluxchat.attachments.disk_visibility', 'private');
        ...
        $attachment = Attachment::first();
        $visibility = Storage::disk('s3')->getVisibility(FluxChat::storageFolder().'/'.$attachment->file_anme);
    ...
    test('it saves file to databse when created & clears files properties when done', function () {
        ...
        config::set('fluxchat.attachments.storage_disk', 'public');
        Storage::fake(config('fluxchat.attachments.storage_disk', 'public'));
        $auth = User::factory()->create();
        $receiver = User::factory()->create(['name' => 'John']);
        $conversation = Conversation::factory()
            ->withParticipants([$auth, $receiver])
...
describe('Deleting Conversation', function () {
    ...
    test('it redirects to chats route after deleting conversation', function () {
        ...
        $request
            ...
            ->assertStatus(200)
            ->assertRedirect(route(FluxChat::indexRouteName()));
    });
    ...
    test('Logged in user can still access deleted conversation in chat route or chatbox', function () {
        ...
        // assert chat route
        $this->actingAs($auth)->get(route(FluxChat::viewRouteName(), $conversation->id))->assertStatus(200);
    });
...
describe('deleteMessage ForEveryone', function () {
    ...
    test('it deletes attachment file from folder when message is deleted ', function () {
        ...
        Storage::disk(config('fluxchat.attachments.storage_disk', 'public'))->assertMissing($attachmentModel->file_name);
    });
