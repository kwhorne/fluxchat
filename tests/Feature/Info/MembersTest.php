...
describe('actions test', function () {
    ...
    describe('sendMessage: ', function () {
        test('it redirects to chat route and does not dispatch "closeFluxChatModal" & "open-chat"  & "close-chat" event when componnet is not Wdiget route after creating conversation ', function () {
            $auth = User::factory()->create();
            $conversation = $auth->createGroup('My Group');
            ...
            $request
                ...
                ->assertRedirect(route(FluxChat::viewRouteName(), 2))
                ->assertNotDispatched('close-chat')
                ->assertNotDispatched('closeFluxChatModal')
                ->assertNotDispatched('open-chat');
        });

        test('it dispatches "closeFluxChatModal" & "open-chat"  & "close-chat" event and does not redirects to chat route and does not when componnet  is Wdiget route after creating conversation ', function () {
            $auth = User::factory()->create();
            $conversation = $auth->createGroup('My Group');
            ...
            $request
                ->call('sendMessage', $participant->id)
                ->assertNoRedirect(route(FluxChat::viewRouteName(), 2))
                ->assertDispatched('open-chat')
                ->assertDispatched('closeFluxChatModal')
                ->assertNotDispatched('close-chat');
