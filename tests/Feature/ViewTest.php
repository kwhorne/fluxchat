...
it('redirects to login page if guest user tries to access chats page ', function () {
    ...
    $response = $this->get(route(FluxChat::viewRouteName(), $conversation->id));
...
test('authenticaed user can access chats page ', function () {
    ...
    // dd($conversation);
    $this->actingAs($auth)->get(route(FluxChat::viewRouteName(), $conversation->id))
        ->assertStatus(200);
...
test('it renders livewire Chat component', function () {
    ...
    $this->actingAs($auth)->get(route(FluxChat::viewRouteName(), $conversation->id))->assertSeeLivewire(Chat::class);
...
test('returns 404 if conversation is not found', function () {
    ...
    // $conversation =  Conversation::factory()->withParticipants([$auth])->create();
    // dd($conversation);
    $this->actingAs($auth)->get(route(FluxChat::viewRouteName(), 1))
        ->assertStatus(404);
