...
class Group extends ModalComponent
{
    ...
    {
        return [
            'name.required' => __('fluxchat::validation.required', ['attribute' => __('fluxchat::chat.group.info.inputs.name.label')]),
            'name.max' => __('fluxchat::validation.max.string', ['attribute' => __('fluxchat::chat.group.info.inputs.name.label')]),
    ...
    public function toggleMember($id, string $class)
    {
        ...
        if ($model) {
            if ($this->selectedMembers->contains(fn ($member) => $member->id == $model->id && get_class($member) == get_class($model))) {
                ...
                if (count($this->selectedMembers) >= FluxChat::maxGroupMembers()) {
    ...
    public function create()
    {
        ...
        $this->handleComponentTermination(
            redirectRoute: route(FluxChat::viewRouteName(), [$conversation->id]),
            events: [
                WidgetsFluxChat::class => ['open-chat',  ['conversation' => $conversation->id]],
            ]
        );
    ...
    public function render()
    {
        ...
        return view('fluxchat::livewire.new.group', ['maxGroupMembers' => FluxChat::maxGroupMembers()]);
    }
}
