...
use Kwhorne\FluxChat\Enums\ConversationType;
use Kwhorne\FluxChat\Enums\MessageType;
use Kwhorne\FluxChat\Events\MessageCreated;
use Kwhorne\FluxChat\Events\MessageDeleted;
use Kwhorne\FluxChat\Facades\FluxChat;
use Kwhorne\FluxChat\Jobs\NotifyParticipants;
...
class Chat extends Component
{
    ...
    public function getListeners()
    {
        ...
        return [
            'refresh' => '$refresh',
            'echo-private:conversation.'.$conversationId.',.Kwhorne\\FluxChat\\Events\\MessageCreated' => 'appendNewMessage',
            'echo-private:conversation.'.$conversationId.',.Kwhorne\\FluxChat\\Events\\MessageDeleted' => 'removeDeletedMessage',

            //  'echo-private:conversation.' .$this->conversation->id. ',.Kwhorne\\FluxChat\\Events\\MessageDeleted' => 'removeDeletedMessage',
        ];
    }
    ...
    {
        return [
            'body.required' => __('fluxchat::validation.required', ['attribute' => __('fluxchat::chat.inputs.message.label')]),
            'media.max' => __('fluxchat::validation.max.array', ['attribute' => __('fluxchat::chat.inputs.media.label')]),
            'media.*.max' => __('fluxchat::validation.max.file', ['attribute' => __('fluxchat::chat.inputs.media.label')]),
            'media.*.mimes' => __('fluxchat::validation.mimes', ['attribute' => __('fluxchat::chat.inputs.media.label')]),
            'files.max' => __('fluxchat::validation.max.array', ['attribute' => __('fluxchat::chat.inputs.files.label')]),
            'files.*.max' => __('fluxchat::validation.max.file', ['attribute' => __('fluxchat::chat.inputs.files.label')]),
            'files.*.mimes' => __('fluxchat::validation.mimes', ['attribute' => __('fluxchat::chat.inputs.files.label')]),
    ...
    public function exitConversation()
    {
        ...
        abort_unless($this->conversation->isGroup(), 403, __('fluxchat::chat.messages.cannot_exit_self_or_private_conversation'));

        // make sure owner if group cannot be removed from chat
        abort_if($auth->isOwnerOf($this->conversation), 403, __('fluxchat::chat.messages.owner_cannot_exit_conversation'));
        ...
        if ($this->isWidget()) {
            ...
            $this->redirectRoute(FluxChat::indexRouteName());
        }
    }
    ...
    protected function rateLimit()
    {
        ...
        if (RateLimiter::tooManyAttempts('send-message:'.auth()->id(), $perMinute)) {
            ...
            return abort(429, __('fluxchat::chat.messages.rate_limit'));
        }
    ...
    public function sendMessage()
    {
        ...
        if (count($attachments) != 0) {
            ...
            $maxUploads = config('fluxchat.attachments.max_uploads');

            // Files
            $fileMimes = implode(',', config('fluxchat.attachments.file_mimes'));
            $fileMaxUploadSize = (int) config('fluxchat.attachments.file_max_upload_size');

            // media
            $mediaMimes = implode(',', config('fluxchat.attachments.media_mimes'));
            $mediaMaxUploadSize = (int) config('fluxchat.attachments.media_max_upload_size');
            ...
            try {
                ...
                return $this->dispatch('fluxchat-toast', type: 'warning', message: $th->getMessage());
            }
            ...
            foreach ($attachments as $key => $attachment) {
                ...
                $path = $attachment->store(
                    FluxChat::storageFolder(),
                    FluxChat::storageDisk()
                );
                ...
                $attachment = $message->attachment()->create([
                    ...
                    'url' => Storage::disk(FluxChat::storageDisk())->url($path), // Use disk and path
                ]);
    ...
    private function messageGroupKey(Message $message): string
    {
        ...
        if ($messageDate->isToday()) {
            $groupKey = __('fluxchat::chat.message_groups.today');
        } elseif ($messageDate->isYesterday()) {
            $groupKey = __('fluxchat::chat.message_groups.yesterday');
    ...
    public function loadMessages()
    {
        ...
        $this->loadedMessages = $messages
            ->groupBy(function ($message) {
                /** @var \Kwhorne\FluxChat\Models\Message $message */
    ...
    {
        return view('fluxchat::components.placeholders.chat');
    }

    public function mount($conversation = null)
    {
        // dd(config('fluxchat.attachments.storage_disk'));
    ...
    private function initializeConversation($conversation)
    {
        ...
        if ($conversation instanceof Conversation) {
            ...
            if (! $this->conversation) {
                abort(404, __('fluxchat::chat.messages.conversation_not_found')); // Custom error response
            }
        } elseif (is_null($conversation)) {
            abort(422, __('fluxchat::chat.messages.conversation_id_required')); // Custom error for missing input
        } else {
            return abort(422, __('fluxchat::chat.messages.invalid_conversation_input')); // Handle invalid input types
        }
    ...
    private function initializeParticipants()
    {
        if (in_array($this->conversation->type, [ConversationType::PRIVATE, ConversationType::SELF])) {
            ...
            /** @var \Kwhorne\FluxChat\Models\Participant|null $participant */
    ...
    {
        return view('fluxchat::livewire.chat.chat');
    }
}
