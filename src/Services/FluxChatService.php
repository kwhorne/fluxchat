...
namespace Kwhorne\FluxChat\Services;

use Illuminate\Support\Facades\Schema;

class FluxChatService
{
    /**
    ...
    {
        return config('fluxchat.color', '#3b82f6');
    }
    ...
    public function searchableFields(): ?array
    {
        ...
        $fieldsToCheck = config('fluxchat.user_searchable_fields');
    ...
    {
        return config('fluxchat.table_prefix');
    }
    ...
    {
        return config('fluxchat.table_prefix').$table;
    }
    ...
    {
        return config('fluxchat.show_new_group_modal_button', false);
    }
    ...
    {
        return config('fluxchat.allow_chats_search', false);
    }
    ...
    {
        return config('fluxchat.show_new_chat_modal_button', false);
    }
    ...
    {
        return (int) config('fluxchat.max_group_members', 1000);
    }

    /**
     * Get the fluxchat storage disk from the configuration.
     *
     * @return string The storage disk.
     */
    public static function storageDisk(): string
    {
        return (string) config('fluxchat.attachments.storage_disk', 'public');
    }

    /**
     * Get the fluxchat storage folder from the configuration.
     *
     * @return string The storage folder.
     */
    public static function storageFolder(): string
    {
        return (string) config('fluxchat.attachments.storage_folder', 'attachments');
    }

    /**
     * Get the fluxchat messages queue from the configuration.
     *
     * @return string The messages queue.
     */
    public static function messagesQueue(): string
    {
        return (string) config('fluxchat.broadcasting.messages_queue', 'default');
    }

    /**
     * Get the fluxchat notifications queue from the configuration.
     *
     * @return string The notifications queue.
     */
    public static function notificationsQueue(): string
    {
        return (string) config('fluxchat.broadcasting.notifications_queue', 'default');
    }
    ...
    {
        return (bool) config('fluxchat.notifications.enabled', false);
    }
    ...
    {
        return (bool) config('fluxchat.uuids', false);
    }
}
