...
<div
    {{ $attributes->merge([
            'class' => "shrink-0 inline-flex items-center justify-center   relative transition
                          overflow-visible rounded-full border border-[var(--fc-light-secondary)] text-gray-300  bg-[var(--fc-light-secondary)] dark:bg-[var(--fc-dark-secondary)]
                          dark:border-[var(--fc-dark-secondary)]  text-base ",
        ])->class(
    ...
    @if (!$src && $group==true)
        ...
        <svg class="shrink-0 p-px w-full h-full rounded-full text-gray-400 dark:text-gray-300 bg-gray-100 bg-[var(--fc-light-secondary)] dark:bg-[var(--fc-dark-secondary)]"
