...
@if(auth()->check() && FluxChat::notificationsEnabled())
   ...
   <div dusk="notification_manager"
        ...
        showNotification(e) {
            ...
            const options = {
                ...
                icon: icon,
                vibrate: [200, 100, 200],
                tag: 'fluxchat-notification-' + message.conversation_id,
                renotify: true,
                data: {
                    url: redirect_url,
        ...
        newNotification(title,options){
          ...
          const notification=   new Notification(title, options);
            ...
            notification.onclick = (event) => {
                ...
                const windowName = 'fluxchat-conversation';
        ...
        registerServiceWorker() {
                if ('serviceWorker' in navigator) {
                    navigator.serviceWorker.register(`{{asset(config('fluxchat.notifcations.main_sw_script','sw.js'))}}`)
        ...
        userId = @js(auth()->id());
        encodedType = @js(\Kwhorne\FluxChat\Helpers\MorphClassResolver::encode(auth()->user()->getMorphClass()));

        {{-- We listen to notify participant event --}}
        Echo.private(`participant.${encodedType}.${userId}`)
            .listen('.Kwhorne\\FluxChat\\Events\\NotifyParticipant', (e) => {
        ...
        document.addEventListener('chat-opened', (event) => {
            ...
            const tag = 'fluxchat-notification-' + conversation;
