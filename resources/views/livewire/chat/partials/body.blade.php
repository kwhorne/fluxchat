...
<main x-data="{
    ...
    @if ($loadedMessages)
        ...
        @foreach ($loadedMessages as $date => $messageGroup)
            ...
            {{-- Date  --}}
            <div  class="sticky top-0 uppercase p-2 shadow-xs px-2.5 z-50 rounded-xl border dark:border-[var(--fc-dark-primary)] border-[var(--fc-light-primary)] text-sm flex text-center justify-center  bg-[var(--fc-light-secondary)] dark:bg-[var(--fc-dark-secondary)] dark:text-white  w-28 mx-auto ">
                {{ $date }}
            </div>
                ...
                <div class="flex gap-2" wire:key="message-{{ $key }}"  >
                    ...
                    @if (!$belongsToAuth && !$isPrivate)
                        <div @class([
                            ...
                            <x-fluxchat::avatar src="{{ $message->sendable?->cover_url ?? null }}" class="h-8 w-8" />
                        </div>
                    @endif
                        ...
                        <div @class([
                            ...
                            @if ($parent != null)
                                <div @class([
                                    ...
                                    $sender = $message?->ownedBy($this->auth) 
                                        ? __('fluxchat::chat.labels.you') 
                                        : ($message->sendable?->display_name ?? __('fluxchat::chat.labels.user'));

                                    $receiver = $parent?->ownedBy($this->auth) 
                                        ? __('fluxchat::chat.labels.you') 
                                        : ($parent->sendable?->display_name ?? __('fluxchat::chat.labels.user'));
                                    @endphp

                                    <h6 class="text-xs text-gray-500 dark:text-gray-300 px-2">
                                        @if ($parent?->ownedBy($this->auth) && $message?->ownedBy($this->auth))
                                            {{ __('fluxchat::chat.labels.you_replied_to_yourself') }}
                                        @elseif ($parent?->ownedBy($this->auth))
                                            {{ __('fluxchat::chat.labels.participant_replied_to_you', ['sender' => $sender]) }}
                                        @elseif ($message?->ownedBy($parent->sendable))
                                            {{ __('fluxchat::chat.labels.participant_replied_to_themself', ['sender' => $sender]) }}
                                        @else
                                            {{ __('fluxchat::chat.labels.participant_replied_other_participant', ['sender' => $sender, 'receiver' => $receiver]) }}
                                        @endif
                                    </h6>
                                    ...
                                    <div @class([
                                        'px-1 border-[var(--fc-light-secondary)] dark:border-[var(--fc-dark-accent)] overflow-hidden ',
                                        ' border-r-4 ml-auto' => $belongsToAuth,
                                        ' border-l-4 mr-auto ' => !$belongsToAuth,
                                    ])>
                                        <p
                                            class=" bg-[var(--fc-light-secondary)] dark:text-white  dark:bg-[var(--fc-dark-secondary)] text-black line-clamp-1 text-sm  rounded-full max-w-fit   px-3 py-1 ">
                                            {{ $parent?->body != '' ? $parent?->body : ($parent->hasAttachment() ?  __('fluxchat::chat.labels.attachment') : '') }}
                                        </p>
                                    </div>
                            ...
                            <div @class([
                                ...
                                <div dusk="message_actions" @class([ 'my-auto flex  w-auto  items-center gap-2', 'order-1' => !$belongsToAuth, ])>
                                    ...
                                    <x-fluxchat::dropdown class="w-40" align="{{ $belongsToAuth ? 'right' : 'left' }}"
                                        width="48">
                                        <x-slot name="trigger">
                                            {{-- Dots --}}
                                        ...
                                        <x-slot name="content">
                                            ...
                                            @if ($message->ownedBy($this->auth)|| ($authParticipant->isAdmin() && $isGroup))
                                                <button dusk="delete_message_for_everyone" wire:click="deleteForEveryone('{{ encrypt($message->id) }}')"
                                                    wire:confirm="{{ __('fluxchat::chat.actions.delete_for_everyone.confirmation_message') }}" class="w-full text-start">
                                                    <x-fluxchat::dropdown-link>
                                                        @lang('fluxchat::chat.actions.delete_for_everyone.label')
                                                    </x-fluxchat::dropdown-link>
                                                </button>
                                            @endif
                                            ...
                                            <button dusk="delete_message_for_me" wire:click="deleteForMe('{{ encrypt($message->id) }}')"
                                                wire:confirm="{{ __('fluxchat::chat.actions.delete_for_me.confirmation_message') }}" class="w-full text-start">
                                                <x-fluxchat::dropdown-link>
                                                    @lang('fluxchat::chat.actions.delete_for_me.label')
                                                </x-fluxchat::dropdown-link>
                                            </button>
                                            @endif
                                            ...
                                            <button dusk="reply_to_message_button" wire:click="setReply('{{ encrypt($message->id) }}')"class="w-full text-start">
                                                <x-fluxchat::dropdown-link>
                                                    @lang('fluxchat::chat.actions.reply.label')
                                                </x-fluxchat::dropdown-link>
                                            </button>
                                ...
                                <div class="flex flex-col gap-2 max-w-[95%]  relative">
                                    ...
                                    @if ($attachment)
                                        @if (!$belongsToAuth && $isGroup)
                                            <div style="color:  var(--fc-brand-primary);" @class([
                                        ...
                                        @if (str()->startsWith($attachment->mime_type, 'application/'))
                                            @include('fluxchat::livewire.chat.partials.file', [ 'attachment' => $attachment ])
                                        @endif

                                        {{-- Attachemnt is Video/ --}}
                                        @if (str()->startsWith($attachment->mime_type, 'video/'))
                                            <x-fluxchat::video height="max-h-[400px]" :cover="false" source="{{ $attachment?->url }}" />
                                        @endif

                                        {{-- Attachemnt is image/ --}}
                                        @if (str()->startsWith($attachment->mime_type, 'image/'))
                                            @include('fluxchat::livewire.chat.partials.image', [ 'previousMessage' => $previousMessage, 'message' => $message, 'nextMessage' => $nextMessage, 'belongsToAuth' => $belongsToAuth, 'attachment' => $attachment ])
                                        @endif
                                    @endif
                                    ...
                                    @include('fluxchat::livewire.chat.partials.message', [ 'previousMessage' => $previousMessage, 'message' => $message, 'nextMessage' => $nextMessage, 'belongsToAuth' => $belongsToAuth, 'isGroup' => $isGroup, 'attachment' => $attachment])
                                    @endif
