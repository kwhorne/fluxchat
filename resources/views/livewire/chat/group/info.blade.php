<div id="group-info-modal" class="bg-[var(--fc-light-primary)] dark:bg-[var(--fc-dark-primary)]     min-h-screen">
    ...
    <section class="cursor-pointer flex gap-4 z-10  items-center p-5 sticky top-0 bg-[var(--fc-light-primary)] dark:bg-[var(--fc-dark-primary)]  ">
        ...
        <h3>{{__('fluxchat::chat.group.info.heading.label')}}</h3>
    </section>
    {{-- Details --}}
    <header>
        ...
        @if ($authIsAdminInGroup || $group?->allowsMembersToEditGroupInfo())
            <div @dusk="edit_group_information_section" class="flex  flex-col items-center gap-5 py-5  px-4    ">
                ...
                <section class="mx-auto items-center justify-center grid">
                    <div @dusk="edit_avatar_label" class="relative  h-32 w-32 overflow-clip mx-auto rounded-full">
                        ...
                        <label wire:target="photo" wire:loading.class="cursor-not-allowed" for="photo"
                            ...
                            <x-fluxchat::avatar wire:loading.class="cursor-not-allowed" group="{{ $isGroup }}"
                ...
                <div class="space-y-3 grid  overflow-x-hidden">
                    ...
                    <form @dusk="edit_group_name_form" wire:submit="updateGroupName" x-data="{ editing: false }"
                        ...
                        <div class="   max-w-[90%] grid h-auto">
                            ...
                            <input x-cloak maxlength="110" x-show="editing" id='groupName' type="text"
                                ...
                                class="resize-none text-2xl font-medium  border-0 px-0 py-0 py-0 border-b border-[var(--fc-light-border)] dark:border-[var(--fc-dark-border)]  bg-inherit dark:text-white outline-hidden w-full focus:outline-hidden  focus:ring-0 hover:ring-0">
                    ...
                    {{-- Members count --}}
                    <p class="mx-auto">  {{ __('fluxchat::chat.group.info.labels.members') }} {{  $totalParticipants }} </p>
                        ...
                        <span class="col-span-11">
                            <div x-show="!editing">
                                @if (empty($description))
                                    <p class="text-sm" style="color: var(--fluxchat-primary-color)">{{ __('fluxchat::chat.group.info.labels.add_description') }}  </p>
                                @else
                            ...
                            <textarea x-cloak maxlength="501" x-show="editing" id='description' type="text" wire:model.blur='description'
                                class="resize-none font-medium w-full border-0 px-0 py-0 py-0 border-b border-[var(--fc-light-border)] dark:border-[var(--fc-dark-border)] bg-inherit dark:text-white outline-hidden w-full focus:outline-hidden  focus:ring-0 hover:ring-0">
                            </textarea>
        ...
        @else
            ...
            <div @dusk="non_editable_group_information_section" class="flex  flex-col items-center gap-5 py-5 px-4  ">
                <x-fluxchat::avatar :src="$cover_url" class=" h-32 w-32 mx-auto" />
    ...
    <section class="my-4 text-left space-y-3">
        ...
        <x-fluxchat::actions.open-modal component="fluxchat.chat.group.members"
            ...
            <button class="cursor-pointer flex w-full justify-between items-center px-8 focus:outline-hidden ">
                <span class="text-gray-600 dark:text-gray-300">{{ __('fluxchat::chat.group.info.labels.members') }}  {{ $totalParticipants }}</span>
                {{-- Search icon --}}
                <span>
        ...
        @if ($authIsAdminInGroup || $group?->allowsMembersToAddOthers())
            <x-fluxchat::actions.open-modal component="fluxchat.chat.group.add-members"
                conversation="{{ $conversation?->id }}" widget="{{ $this->isWidget() }}">
                <button @dusk="open_add_members_modal_button"
                    class="cursor-pointer w-full py-5 px-8 hover:bg-[var(--fc-light-secondary)] dark:hover:bg-[var(--fc-dark-secondary)] focus:outline-hidden transition  flex gap-3 items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                        class="size-6 w-5 h-5">
                        <path
                    ...
                    <span>{{ __('fluxchat::chat.group.info.actions.add_members.label') }}</span>
                </button>
            </x-fluxchat::actions.open-modal>
        @endif
    ...
    <footer class="flex flex-col justify-start w-full">
        ...
        @if ($authIsOwner)
            ...
            {{-- Delete group --}}
            <button wire:confirm="{{ __('fluxchat::chat.group.info.actions.delete_group.confirmation_message') }}" wire:click="deleteGroup"
                class="cursor-pointer w-full py-5 px-8 hover:bg-[var(--fc-light-secondary)] dark:hover:bg-[var(--fc-dark-secondary)] transition text-start space-y-2   gap-3   text-red-500">
                <div class="flex gap-3 items-center ">
                    ...
                    <span>{{ __('fluxchat::chat.group.info.actions.delete_group.label') }}</span>
                </div>

                <p class="dark:text-white/60 text-sm text-gray-600/80">@lang('fluxchat::chat.group.info.actions.delete_group.helper_text')</p>
            </button>
            {{-- Permissions --}}
            <div>

                <x-fluxchat::actions.open-chat-drawer component='fluxchat.chat.group.permissions'
                    conversation="{{ $conversation?->id }}">
                    <button
                        class="cursor-pointer w-full py-5 px-8 hover:bg-[var(--fc-light-secondary)] dark:hover:bg-[var(--fc-dark-secondary)] transition text-start space-y-2   gap-3   dark:text-white/90">
                        <div class="flex gap-3 items-center ">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            ...
                            <span>@lang('fluxchat::chat.group.info.actions.group_permissions.label')</span>
                        </div>
                    </button>
        ...
        @else
        {{-- Exit Group --}}
            <button wire:confirm="{{ __('fluxchat::chat.group.info.actions.exit_group.confirmation_message') }}" wire:click="exitConversation"
                class="cursor-pointer w-full py-5 px-8 hover:bg-[var(--fc-light-secondary)] dark:hover:bg-[var(--fc-dark-secondary)] transition flex gap-3 items-center text-red-500">
                ...
                <span>@lang('fluxchat::chat.group.info.actions.exit_group.label')</span>
            </button>
        @endif
