<div class="h-[calc(100vh_-_10rem)]  sm:h-[450px] bg-[var(--fc-light-primary)] dark:bg-[var(--fc-dark-primary)] dark:text-white border border-[var(--fc-light-secondary)] dark:border-[var(--fc-dark-secondary)] overflow-y-auto overflow-x-hidden  ">
...
<header class=" sticky top-0 bg-[var(--fc-light-primary)] dark:bg-[var(--fc-dark-primary)] z-10 p-2">
    <div class="flex items-center pb-2">
        ...
        <h3  class="text-sm mx-auto font-semibold "  ><span>{{__('fluxchat::chat.group.add_members.heading.label')}} </span> {{$newTotalCount}} / {{$maxGroupMembers}}</h3>
        ...
        <button 
            ...
            @class([
                'p-2 disabled:cursor-not-allowed curosr-pointer disabled:hover:bg-inherit ml-0 text-gray-600 dark:text-gray-300  hover:bg-[var(--fc-light-secondary)] hover:dark:bg-[var(--fc-dark-secondary)] dark:hover:text-white rounded-full hover:text-gray-800',
                'cursor-not-allowed hover:bg-none dark:hover:bg-inherit hover:bg-inherit  opacity-70'=>count($selectedMembers)==0
            ])
            class="">
            {{__('fluxchat::chat.group.add_members.actions.save.label')}}
        </button>
    ...
    <div
      ...
      "
       ...
       <span x-transition x-show="showError">
        {{__('fluxchat::chat.group.add_members.messages.members_limit_error',['count'=>$maxGroupMembers])}}
       </span>
    </div>
    ...
    <section class="flex flex-wrap items-center px-0 border-b border-[var(--fc-light-secondary)] dark:border-[var(--fc-dark-secondary)]">
        <input type="search" id="users-search-field" wire:model.live.debounce='search' autocomplete="off"
            placeholder="{{ __('fluxchat::chat.group.add_members.inputs.search.placeholder') }}"
         ...
         class="flex w-full overflow-x-auto gap-3">
            ...
            @if ($selectedMembers)
                ...
                @foreach ($selectedMembers as $key => $member)
                    <li class="flex items-center text-nowrap min-w-fit px-2 py-1 text-sm font-medium text-gray-800 bg-[var(--fc-light-secondary)] rounded-sm dark:bg-[var(--fc-dark-secondary)] dark:text-gray-300"
                           ...
                           <div @class(['opacity-70' => $isAlreadyAParticipant]) >
                            ...
                            @class(['transition-all truncate', 'group-hover:underline ' => !$isAlreadyAParticipant])>
                             ...
                             @class(['text-gray-600 dark:text-gray-400 text-sm'])>
                                @if ($isAlreadyAParticipant)
                                {{__('fluxchat::chat.group.add_members.messages.member_already_exists')}}
                                @endif
                             </span>
                           </div>
