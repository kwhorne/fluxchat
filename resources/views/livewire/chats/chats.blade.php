...
<div x-data="{ selectedConversationId: '{{ request()->conversation ?? $selectedConversationId }}' }"
    ...
    class="flex flex-col bg-[var(--fc-light-primary)] dark:bg-[var(--fc-dark-primary)] transition-all h-full overflow-hidden w-full sm:p-3">
    ...
    {{-- include header --}}
    @includeWhen($showHeader, 'fluxchat::livewire.chats.partials.header')
    ...
    <main x-data
        ...
        @if (count($conversations) > 0)
            {{-- include list item --}}
            @include('fluxchat::livewire.chats.partials.list')


            {{-- include load more if true --}}
            @includeWhen($canLoadMore, 'fluxchat::livewire.chats.partials.load-more-button')
        @else
            <div class="w-full flex items-center h-full justify-center">
                <h6 class=" font-bold text-gray-700 dark:text-white">{{ __('fluxchat::chats.labels.no_conversations_yet')  }}</h6>
            </div>
        @endif
    </main>
