...
<ul wire:loading.delay.long.remove wire:target="search" class="p-2 grid w-full spacey-y-2">
    ...
    id="conversation-{{ $conversation->id }}" 
        ...
        @else
        wire:navigate href="{{ route(FluxChat::viewRouteName(), $conversation->id) }}" @endif
            @style(['border-color:var(--fc-brand-primary)' => $selectedConversationId == $conversation?->id])
            class="py-3 flex gap-4 dark:hover:bg-[var(--fc-dark-secondary)]  hover:bg-[var(--fc-light-secondary)]  rounded-xs transition-colors duration-150  relative w-full cursor-pointer px-2"
            :class="$wire.selectedConversationId == conversationID &&
                'dark:bg-[var(--fc-dark-secondary)] bg-[var(--fc-light-secondary)] border-r-4  border-opacity-20 border-[var(--fc-brand-primary)]'">
            ...
            <aside class="grid  grid-cols-12 w-full">
                <div
                    class="col-span-10 border-b pb-2 border-[var(--fc-light-border)] dark:border-[var(--fc-dark-border)] relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
                    ...
                    <div class="flex gap-1 mb-1 w-full items-center">
                        ...
                        @if ($conversation->isSelfConversation())
                            <span class="font-medium dark:text-white">({{__('fluxchat::chats.labels.you')  }})</span>
                        @endif
                    ...
                    {{-- Message body --}}
                    @if ($lastMessage != null)
                        @include('fluxchat::livewire.chats.partials.message-body')
                    @endif
                ...
                @if ($lastMessage != null && !$lastMessage?->ownedBy($this->auth) && !$isReadByAuth)
                    <div x-show="showUnreadStatus" dusk="unreadMessagesDot" class=" col-span-2 flex flex-col text-center my-auto">
                        ...
                        <svg @style(['color:var(--fc-brand-primary)']) xmlns="http://www.w3.org/2000/svg" width="16" height="16"
