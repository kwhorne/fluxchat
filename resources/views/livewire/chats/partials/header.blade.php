...
<header class="px-3 z-10 sticky top-0 w-full py-2 " dusk="header">
    ...
    <section class=" justify-between flex items-center   pb-2">
        ...
        <div class="flex gap-x-3 items-center  ">
            ...
            <x-fluxchat::actions.new-chat widget="{{$this->isWidget()}}">
            ...
            </x-fluxchat::actions.new-chat>
            @endif


            {{-- Only show if is not widget --}}
            @if ($showHomeRouteButton)
            <a id="redirect-button" href="{{ config('fluxchat.home_route', '/') }}" class="flex items-center">
    ...
    @if ($allowChatsSearch)
        <section class="mt-4">
            <div class="px-2 rounded-lg dark:bg-[var(--fc-dark-secondary)]  bg-[var(--fc-light-secondary)]  grid grid-cols-12 items-center">
                ...
                <input id="chats-search-field" name="chats_search" maxlength="100" type="search" wire:model.live.debounce='search'
                    placeholder="{{ __('fluxchat::chats.inputs.search.placeholder')  }}" autocomplete="off"
