<div class="flex gap-x-2 items-center">
    ...
    @if ($belongsToAuth)
        <span class="font-bold text-xs dark:text-white/90 dark:font-normal">
            @lang('fluxchat::chats.labels.you'):
        </span>
    ...
    <p @class([
        ...
        {{ $lastMessage->body != '' ? $lastMessage->body : ($lastMessage->isAttachment() ? '📎 '.__('fluxchat::chats.labels.attachment') : '') }}
    </p>
    ...
    <span class="font-medium px-1 text-xs shrink-0 text-gray-800 dark:text-gray-50">
        @if ($lastMessage->created_at->diffInMinutes(now()) < 1)
          @lang('fluxchat::chats.labels.now')
        @else
